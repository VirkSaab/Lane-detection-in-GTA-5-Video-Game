...
def compute_binary_image(color_image):
    """
    ...
    # get yellow lanes 
    lower_yellow = np.array([0,50, 150])
...
def lane_fill_poly(binary_warped,undist, inverse_perspective_transform, left_fit,right_fit):
...
def image_processing(image):
    ...
    binary_img = compute_binary_image(bgr_image)
    ...
    roi_img = region_of_interest(binary_img, vertices)

    #Transform image prespective to birds eye view
    warped_img, inverse_perspective_transform = compute_perspective_transform(binary_img)

    if window_search:
        #window search
        left_fit,right_fit = track_lanes_initialize(warped_img)
        #store values
        ...
        left_fit,right_fit,leftx,lefty,rightx,righty = track_lanes_update(warped_img, left_fit,right_fit)
    ...
    #draw polygon
    processed_frame = lane_fill_poly(warped_img, bgr_image, inverse_perspective_transform, left_fit, right_fit)
...
def main(monitor={'top': 40, 'left': 0, 'width': 800, 'height': 600}):
    global window_search 

    window_search = True
    time.sleep(5)
    # cv2.namedWindow("OpenCV-capture")
    # cv2.setMouseCallback("OpenCV-capture", draw_circle)
    ...
    with mss.mss() as sct:
        while 'Screen capturing':
            ...
            # Display the picture
            cv2.imshow('OpenCV-capture', processed_img)

            # Display the picture in grayscale
            # cv2.imshow('OpenCV-capture', cv2.cvtColor(processed_img, cv2.COLOR_BGRA2RGB))
